...
class PiStomp(Component):
    ...
    def build_and_install(self, source_dir: Path):
        ...
        (user_home / "data/user-files").mkdir(parents=True, exist_ok=True) # Usually separate clone

        # Install Config Templates
        run_cmd(f"install -m 644 {source_dir}/setup/config_templates/default_config.yml {user_home}/data/config/", shell=True)
        run_cmd(f"install -m 644 {source_dir}/setup/config_templates/default-hardware-descriptor.json {user_home}/data/config/", shell=True)
        ...
        run_cmd(f"install -m 755 {source_dir}/setup/services/hotspot/usr/lib/pistomp-wifi/disable_wifi_hotspot.sh {wifi_lib}", shell=True)
        run_cmd(f"install -m 755 {source_dir}/setup/services/hotspot/usr/lib/pistomp-wifi/enable_wifi_hotspot.sh {wifi_lib}", shell=True)
        run_cmd(f"install -m 755 {source_dir}/setup/services/wifi_check.sh {wifi_lib}", shell=True)
        
        # Wifi service
        run_cmd(f"install -m 644 {source_dir}/setup/services/hotspot/usr/lib/systemd/system/wifi-hotspot.service /usr/lib/systemd/system/", shell=True)
        run_cmd(f"chown -R {first_user}:{first_user} {wifi_lib}", shell=True)
        
        # Helper scripts
        run_cmd(f"install -m 755 {source_dir}/setup/mod-tweaks/start_touchosc2midi.sh /usr/mod/scripts/", shell=True)
        ...
        run_cmd("ln -sf /usr/lib/systemd/system/mod-ala-pi-stomp.service /etc/systemd/system/multi-user.target.wants/mod-ala-pi-stomp.service", shell=True)
